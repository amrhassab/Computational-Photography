...
imagepath = 'my_input_folder';   % Assumes images are in order and are the *only*
...
for i=1:length(imagelist)-1
    ...
    %% STEP 3A: Calculate Homographies without RANSAC
    
    H_noRANSAC{i} = calcH_noRansac(im1_ftr_pts, im2_ftr_pts);
    
    %% STEP 3B: Calaculate Homagraphies with RANSAC
...
img1 = im2double(imread('input_images/1.jpg')); % <- reference image
img2 = im2double(imread('input_images/2.jpg'));
img3 = im2double(imread('input_images/3.jpg'));
img4 = im2double(imread('input_images/4.jpg'));

H11 = eye(3);
H21 = H_list{1};
H31 = H21 * H_list{2}; % H21 * H32
H41 = H31 * H_list{3}; % H21 * H32 * H43
...
%{
    im_alpha = im2double(bwdist(im2bw(warped_images{1}), 'euclidean'));
    im_alpha = im_alpha - min(min(im_alpha));
    im_alpha = im_alpha + .001;
    im_alpha = im_alpha / max(max(im_alpha));
%}


%panorama_image = blend(panorama_image, warped_images{1});

%for i = 2 : length(warped_images)    
%for i = 2 : 4
%panorama_image = blend(panorama_image, blend(warped_images{i-1}, warped_images{i}));
    %panoram
%end

panorama_image = blend(warped_images{4},blend(warped_images{3},blend(warped_images{2}, warped_images{1})));
